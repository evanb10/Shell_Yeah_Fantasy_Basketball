...
from .forms import user_name_form, odds_form, league_selection_form
...
def user_leagues(request, user_name):
    ...
    for league in user_leagues_list:
        ...
        # (Make sure that league['avatar'] is the Base64-encoded string of the PNG image.)
        label_html = (
            f'<img src="data:image/png;base64,{league["avatar"]}" '
            f'alt="{league["name"]} avatar" width="50" height="50"> '
            f'{league["name"]}'
        )
        choices.append((league['league_id'], mark_safe(label_html)))
    
    if request.method == 'POST':
        # Bind the form and re-populate the choices.
    ...
    return render(request, 'user_leagues.html', {'form': form})
